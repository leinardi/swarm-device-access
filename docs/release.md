...
How a swarm-device-access release is versioned, built and published, what to do when one fails halfway, and what has to be set
...
- Two static binaries, `swarm-device-access-linux-amd64` and `swarm-device-access-linux-arm64`, built with `CGO_ENABLED=0`,
...
- A multi-arch (`linux/amd64`, `linux/arm64`) image at `ghcr.io/leinardi/swarm-device-access:<version>`, with an SBOM and
...
Run the **Release swarm-device-access** workflow from the Actions tab (or `gh workflow run release.yaml`) on `master`. It takes two
inputs:
...
Tags in this repository are immutable. The "Immutable tags" ruleset covers `~ALL` tags except `v[0-9]`, `v[1-9][0-9]` and
...
and the artifact list; `dist/` is uploaded as a workflow artifact. A dry run with no version is also the quickest way to see what
...
Either way the tag check sees the tag on `HEAD`, the push is skipped, and the run finishes whatever is missing. There is no cleanup
...
- **The image.** A fresh image is pushed by digest with no tag, attested and signed, and only then tagged `:<version>`, so a version
  ...
  `gh attestation verify oci://ghcr.io/leinardi/swarm-device-access@<digest> --repo leinardi/swarm-device-access --signer-workflow leinardi/swarm-device-access/.github/workflows/release.yaml --source-ref refs/heads/master --source-digest <commit>`
...
- **The signature.** An existing signature counts only when it matches this workflow's identity and issuer:
  `cosign verify --certificate-identity https://github.com/leinardi/swarm-device-access/.github/workflows/release.yaml@refs/heads/master --certificate-oidc-issuer https://token.actions.githubusercontent.com`.
...
```bash
IMAGE=ghcr.io/leinardi/swarm-device-access
DIGEST=sha256:...   # the digest of the image that should be latest
gh attestation verify "oci://$IMAGE@$DIGEST" --repo leinardi/swarm-device-access \
